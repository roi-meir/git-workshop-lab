...
- **Rebase** — Replay your commits on top of another branch (`git rebase`); cleaner history, use with care on shared branches.
- **Tags** — Mark a point in history (e.g. a release): `git tag v1.0`.
- **Cherry-pick** — Apply one specific commit from another branch: `git cherry-pick <commit>`.
- **Amend** — Fix the last commit: `git commit --amend` (message or add forgotten files).
- **Reflog** — See where branches and HEAD have been: `git reflog`; helps recover “lost” commits.
- **Blame** — See who changed each line in a file: `git blame <file>`.
